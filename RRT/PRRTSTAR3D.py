...
class RRT:
    ...
    def rrt_star_planning(self, start, goal, animation=True):
        ...
        for i in range(self.max_iter):
            ...
            # random sampling and deflected under artificial potential field
            rnd = self.sample()
    ...
    def sample(self):
        if random.randint(0, 100) >= self.goal_sample_rate:
            ...
            # The sampling point deflects under the artificial potential field
            rnd = self.RGD(rnd)
        else:  # goal point sampling
            rnd = [self.goal.x, self.goal.y, self.goal.z]
        return rnd

    def Att_potential_field(self, rnd):
        dis_Goal_rnd = math.sqrt((self.goal.x - rnd[0]) ** 2 + (self.goal.y - rnd[1]) ** 2 + (self.goal.z - rnd[2]) ** 2)
        Uattx = 0.5*self.Ka*(dis_Goal_rnd)*(self.goal.x - rnd[0])
        Uatty = 0.5*self.Ka*(dis_Goal_rnd)*(self.goal.y - rnd[1])
        Uattz = 0.5*self.Ka*(dis_Goal_rnd)*(self.goal.z - rnd[2])
        return Uattx, Uatty, Uattz

    def RGD(self, rnd):
        prnd = rnd
        dobs = 0.5
        for i in range(self.k):
            Fatt = self.Att_potential_field(prnd)
            dmin = float('inf')
            for obstacle in self.obstacle_list:
                if obstacle[0] < rnd[0] < (obstacle[0]+obstacle[3]):
                    disx = 0
                else:
                    disx = min(abs(obstacle[0] - rnd[0]), abs(obstacle[0]+obstacle[3] - rnd[0]))
                if obstacle[1] < rnd[1] < (obstacle[1]+obstacle[4]):
                    disy = 0
                else:
                    disy = min(abs(obstacle[1] - rnd[1]), abs(obstacle[1]+obstacle[4] - rnd[1]))
                if obstacle[2] < rnd[2] < (obstacle[2]+obstacle[5]):
                    disz = 0
                else:
                    disz = min(abs(obstacle[2] - rnd[2]), abs(obstacle[2]+obstacle[5] - rnd[2]))
            ...
            if dmin < dobs:
                ...
                prnd[0] = prnd[0] + dobs * Fatt[0]/math.sqrt(Fatt[0] ** 2 + Fatt[1] ** 2 + Fatt[2] ** 2)
                prnd[1] = prnd[1] + dobs * Fatt[1] / math.sqrt(Fatt[0] ** 2 + Fatt[1] ** 2 + Fatt[2] ** 2)
                prnd[2] = prnd[2] + dobs * Fatt[2] / math.sqrt(Fatt[0] ** 2 + Fatt[1] ** 2 + Fatt[2] ** 2)
        return prnd
...
def main():
    ...
    path = rrt.rrt_star_planning(start=[1, 1, 5], goal=[45, 45, 10], animation=show_animation)  # mpa5
